...
AStar_Node *AStar_Find(int mapWidth,int mapHeight,int StartX,int StartY,int EndX,int EndY,int (*GetMap)(int,int),NodeDataMap *dataMap)
{
  ...
  AStarNode_List *AllNodesGSet   = NULL;
  ...
  if ((GetMap(StartX,StartY) == 1) || (GetMap(EndX,EndY) == 1))
  {
    ...
    for (neighbor_pos = 0;neighbor_pos < 4;neighbor_pos++)
    {
      ...
      if (!Neighbor)
      {
        ...
        if ((TempNodeToFind.X >= 0) && (TempNodeToFind.X < mapWidth)
            ...
            && (GetMap(TempNodeToFind.X,TempNodeToFind.Y) == 0))
        {
...
int Astar_return(int StartX,int StartY,int EndX,int EndY,int (*CustomGetMap)(int,int)){
    AStar_Node *Solution;
	...
	if (NextInSolution)
  {
    ...
    for(i=1;i<=1000;i++)
    {
      NextInSolutionPos = NextInSolution->X + (NextInSolution->Y * MAP_WIDTH);
		...
		if(dataMap[NextInSolutionPos].CameFrom->X==StartX&&dataMap[NextInSolutionPos].CameFrom->Y==StartY){
			
			temp=(NextInSolution->X+(NextInSolution->Y)*40);
			free(dataMap);
			return temp;
			break;
		}
